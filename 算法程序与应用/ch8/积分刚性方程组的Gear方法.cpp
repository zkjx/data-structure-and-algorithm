...
//hmin        积分过程中允许的最小步长。
//hmax        积分过程中允许的最大步长。
//h           积分的拟定步长。hmin<<h<hmax。
//eps         误差检验常数。
//n           方程个数，也是未知数个数。
//y0[n]       n个未知函数在起始点处的函数值。
//k           拟定输出的积分点数。
//t[k]        返回实际有效输出点（包括起始点）的自变量值。
//z[n][k]     返回实际有效输出点处的未知函数值。
//ss          计算雅可比矩阵的函数名。
//f           计算方程组中各方程右端函数值的函数名。
  ...
  l60:
      ...
      { case 1: aa[0]=-1.0; break;
        ...
        case 6: aa[0]=-720.0/1764.0; aa[2]=-1624.0/1764.0;
                ...
                break;
        default: { cout <<"阶数已大于6，积分停止(前输出点有效)" <<endl;
			       delete[] d; delete[] p; delete[] s; delete[] s02;
                   delete[] ym; delete[] er; delete[] yy; 
                   delete[] y; 
				   return(nn);       //阶数大于6，积分停止
                 }
      }
      m=nq+1; idb=m;
      ...
      if (edwn==0.0)      //精度要求太高，积分停止
      { 
		  cout <<"精度要求太高，积分停止(前输出点有效)" <<endl;
		  delete[] d; delete[] p; delete[] s; delete[] s02;
          delete[] ym; delete[] er; delete[] yy; 
          delete[] y; 
		  return(nn);
  ...
  l80:
      ...
      { 
          ...
          cout <<"对于h>hmin校正迭代不收敛，积分停止(前输出点有效)" <<endl;
		  delete[] d; delete[] p; delete[] s; delete[] s02;
          delete[] ym; delete[] er; delete[] yy; 
          delete[] y; 
		  return(nn);                //h>=hmin校正迭代不收敛,积分停止
      }
      dd=0.0;
      ...
      { 
		  kf=kf-2;
          if (h<=(hmin*1.00001))   //步长已小于hmin，但精度达不到，积分停止
          { 
			  cout <<"步长小于hmin,精度达不到,积分停止(前输出点有效)" <<endl;
		      delete[] d; delete[] p; delete[] s; delete[] s02;
              delete[] ym; delete[] er; delete[] yy; 
              delete[] y; 
			  return(nn);
